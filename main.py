...
def printUsage():
	...
	print("+ python main.py -u USERNAME -p PASSWORD -o follow-listss -t USER_LIST: Follow users from a file with user id")
	print("+ python main.py -u USERNAME -p PASSWORD -o follow-listsss -t USER_LIST: UnFollow users from a file")
...
def follow_listss(target):
	...
	for user_id in user_list:
		...
		api.follow(user_id)
		tot += 1
		print("Following "+str(user_id)+" (with id "+str(user_id)+")")
		if(tot>=MAXIMO):
			break
	print("Total: "+str(tot)+" users followed from "+str(target)+" (Max val: "+str(MAXIMO)+")\n")
	
def follow_listsss(target):
	user_list = open(target).read().splitlines()
	tot = 0
	for user_id in user_list:
		time.sleep(float( random.uniform(min_delay*10,max_delay*10) / 10 ))
		...
		tot += 1
		print("UnFollowing "+str(user_id)+" (with id "+str(user_id)+")")
		if(tot>=MAXIMO):
			break
	print("Total: "+str(tot)+" users unfollowed from "+str(target)+" (Max val: "+str(MAXIMO)+")\n")
...
def main():
	...
	elif(option == "follow-listss"):
		target = args.target
		if target is not None:
			follow_listss(target)
		else:
			printUsage()
			
	elif(option == "follow-listsss"):
		target = args.target
		if target is not None:
			follow_listsss(target)
		else:
			printUsage()
